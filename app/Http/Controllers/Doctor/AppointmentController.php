...
class AppointmentController extends Controller
{
    ...
    public function addNotes(Request $request, Appointment $appointment): RedirectResponse
    {
        ...
        if ($request->boolean('complete')) {
            ...
            // AI PREDICTION - Run when appointment is completed
            $prediction = $this->runHealthTrendPrediction($validated['notes'], $appointment);
            
            if ($prediction) {
                $updateData['ai_trend'] = $prediction['trend'];
                $updateData['ai_confidence'] = $prediction['confidence'];
            }
        }
    ...
    /**
     * Run AI prediction based on doctor's notes
     * 
     * @param string $notes
     * @param Appointment $appointment
     * @return array|null
     */
    private function runHealthTrendPrediction(string $notes, Appointment $appointment): ?array
    {
        try {
            // Get patient's county
            $county = $appointment->patient->patientProfile->county ?? 'Nairobi';
            
            // Default values
            $healthLevel = 'Health Centre';
            $nurseExperience = 5;

            // Paths to Python script and model
            $pythonScript = app_path('Http/Controllers/ai/predict_trends.py');
            $modelPath = app_path('Http/Controllers/ai/health_trends_model.pkl');
